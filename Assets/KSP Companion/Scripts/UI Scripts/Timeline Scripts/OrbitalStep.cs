...
public class OrbitalStep : TimelineStep, IInspectable, IPlottable
{
    ...
    public void OnSelect(BaseEventData eventData)
    {
        ...
        blockTwo.AddDoubleProperty("Inclination (deg)",                 ValueGetter: () => { if (orbit != null) return orbit.INC.DegValue; else return 0.0; },  ValueSetter: newINCd => { if (orbit != null) orbit.INC = Mathd.Clamp(newINCd * Mathd.Deg2Rad, 0.0, Mathd.PI); UpdateAdjacentTransferOrbits(); },    DisplayCondition: () => orbit != null, isEditable: MayUserEditOrbit);
        blockTwo.AddDoubleProperty("Longitude Of Ascending Node (deg)", ValueGetter: () => { if (orbit != null) return orbit.LAN.DegValue; else return 0.0; },  ValueSetter: newLANd => { if (orbit != null) orbit.LAN = newLANd * Mathd.Deg2Rad; UpdateAdjacentTransferOrbits(); },                                DisplayCondition: () => orbit != null, isEditable: MayUserEditOrbit);
        blockTwo.AddDoubleProperty("Argument Of Periapsis (deg)",       ValueGetter: () => { if (orbit != null) return orbit.APE.DegValue; else return 0.0; },  ValueSetter: newAPEd => { if (orbit != null) orbit.APE = newAPEd * Mathd.Deg2Rad; UpdateAdjacentTransferOrbits(); },                                DisplayCondition: () => orbit != null, isEditable: MayUserEditOrbit);
